...
//          } else {
            // removed this fallbackSound because it is not used
            //Couldn't open content://0@settings/system/alarm_alert_cache
...
//        createAlarmNotification(this, alarmUri);
//
//      }else {

        Intent notifyActivity = new Intent(getApplicationContext(), AlarmAlertVideoActivity.class);
        notifyActivity.setData(intent.getData());
        notifyActivity.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        startActivity(notifyActivity);
//      }
    }
  ...
  private void createAlarmNotification(Context context, Uri alarmUri){
       ...
       */
      String ALARM_NOTIFICATION_CHANNEL_ID = "my_channel_01";
