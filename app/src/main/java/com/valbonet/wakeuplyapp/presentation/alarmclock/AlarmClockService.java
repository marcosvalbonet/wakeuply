...
public final class AlarmClockService extends Service {
  ...
  private void refreshNotification() {
    ...
    String CHANNEL_ID = "wakeuply_channel";// The id of the channel.
    ...
    if (pendingAlarms.size() > 0 && AppSettings.displayNotificationIcon(c)) {
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
        ...
        Notification lastNotification = getActiveNotification(NOTIFICATION_BAR_ID);
        if (lastNotification != null){
            String title = lastNotification.extras.getString("android.title");
            String text = lastNotification.extras.getString("android.text");
          if (text != null && text.equals(resolvedString)) {
            return ;
          }
        }

        CharSequence name = "Wakeup.ly alarms";///getString(R.string.channel_name);// The user-visible name of the channel.
      ...
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
        startForeground(NOTIFICATION_BAR_ID, notification);
      } else {
  ...
  public void createAlarm(AlarmTime time) {
        ...
        //TODO MVAL: add the default setting to this alarm
        AlarmSettings alarmSettings = new AlarmSettings();
        db.writeAlarmSettings(alarmId, alarmSettings);
